...
import os
# from src.bullet.douyin.protobuf.douyin import *
...
def generateSignature(wss, script_file=json_path):
    """
...
class DouyinLiveWebFetcher:
    def __init__(self, live_id, cookies=None):
        """
        直播间弹幕抓取对象
        ...
        # If ttwid is in cookies, use it
        if 'ttwid' in self.cookies:
            self.__ttwid = self.cookies['ttwid']
    ...
    def room_id(self):
        ...
        try:
            ...
            response.raise_for_status()
            print(f"Room page response status: {response.status_code}")
            print(f"Room page content length: {len(response.content)}")
            # Try different regex patterns to find roomId
            patterns = [
                r'roomId\\":\\"(\d+)\\"',
                r'room_id\\":\\"(\d+)\\"',
                r'roomId:\s*(\d+)',
            ...
            for pattern in patterns:
                ...
                if match:
                    ...
                    print(f"Found room_id: {self.__room_id} using pattern: {pattern}")
                    return self.__room_id
            
            print("【X】No match found for roomId in any pattern")
    ...
    def get_room_status(self):
        try:
            if not hasattr(self, 'room_id') or not self.room_id:
                raise AttributeError("房间id属性缺失或为空")
            ...
            if resp.content:
                ...
                print("【X】API返回空响应")
                # Try alternative approach - check if room is live by looking at the page content
                live_url = self.live_url + self.live_id
                live_resp = requests.get(live_url, headers={
                    'User-Agent': self.user_agent,
                    'Cookie': f'ttwid={self.ttwid};'
                })
                if '正在直播' in live_resp.text:
                    print("【√】直播间正在直播")
                elif '直播已结束' in live_resp.text:
                    print("【√】直播已结束")
                else:
                    print("【?】无法确定直播状态")
            return 
    ...
    def _connectWebSocket(self):
        ...
        try:
            if not hasattr(self, 'room_id') or not self.room_id:
                return AttributeError("room_id属性缺失或为空")
            ...
            print(f"WebSocket连接发生未知错误: {str(e)}")
            self.stop()
            raise
    ...
    def _parseMemberMsg(self, payload):
        ...
        # Store or process member data as needed
        if self.message_callback:
            self.message_callback(member_data)
